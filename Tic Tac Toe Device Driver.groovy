 ...
 *
 *  The Tic Tac Toe grid consists of columns A, B, and C and rows 1, 2, and 3 (referenced as attributes: A1, A2, A3, B1, B2, B3, C1, C2, C3). The
 *  initial value for each square in the grid is set to "-" (meaning it has not yet been selected). As each player selects squares, they will be set
 *  to "X" or "O" depending on which player selected the square.
 *
 ...
 *
 *  The driver automatically starts the game with the "X" player, automatically alternating between "X" and "O". To select a square as the current
 *  player, initiate a button press on the virtual device (e.g., "push button 1" to select square A1).  The button numbers for each square
